...
import { TipoDeduccionDialogComponent } from './tipo-deduccion-dialog.component';
      ...
      <mat-card>
        ...
        <mat-card-content>
          ...
          @if (isLoading()) {
            ...
            @if (tiposDeduccionFiltrados().length === 0) {
              <div class="empty-state">
                <mat-icon>inbox</mat-icon>
                <p>No se encontraron tipos de deducción</p>
              </div>
            }

            <mat-paginator
              [length]="tiposDeduccionFiltrados().length"
              [pageSize]="pageSize"
              [pageIndex]="pageIndex"
...
export class TiposDeduccionComponent implements OnInit {
  private http = inject(HttpClient);
  private snackBar = inject(MatSnackBar);
  private dialog = inject(MatDialog);

  tiposDeduccion = signal<TipoDeduccion[]>([]);
  tiposDeduccionFiltrados = signal<TipoDeduccion[]>([]);
  ...
  loadData(): void {
    ...
    this.http.get<TipoDeduccion[]>(`${environment.apiUrl}/tipos-deduccion`).subscribe({
      next: (data) => {
        this.tiposDeduccion.set(data);
        this.aplicarFiltros();
        this.isLoading.set(false);
      },
      error: (error) => {
        console.error('Error al cargar tipos de deducción:', error);
        this.snackBar.open('Error al cargar tipos de deducción', 'Cerrar', {
          duration: 3000,
        });
        this.isLoading.set(false);
      },
  ...
  aplicarFiltros(): void {
    let resultado = [...this.tiposDeduccion()];
    ...
    this.tiposDeduccionFiltrados.set(resultado);
  }

  private getValorOrdenamiento(item: TipoDeduccion): any {
  ...
  getDatosPaginados(): TipoDeduccion[] {
    const inicio = this.pageIndex * this.pageSize;
    const fin = inicio + this.pageSize;
    return this.tiposDeduccionFiltrados().slice(inicio, fin);
  }

  openDialog(tipoDeduccion?: TipoDeduccion): void {
    const dialogRef = this.dialog.open(TipoDeduccionDialogComponent, {
      width: '600px',
      maxWidth: '90vw',
      data: { tipoDeduccion: tipoDeduccion || null },
      disableClose: true,
    });
  ...
  toggleActivo(tipoDeduccion: TipoDeduccion): void {
    const nuevoEstado = !tipoDeduccion.activo;
    this.http
      .put<TipoDeduccion>(`${environment.apiUrl}/tipos-deduccion/${tipoDeduccion.id}`, {
        activo: nuevoEstado,
      })
      .subscribe({
        next: () => {
          this.snackBar.open(
            `Tipo de deducción ${nuevoEstado ? 'activado' : 'desactivado'} correctamente`,
            'Cerrar',
            { duration: 3000 }
          );
          this.loadData();
