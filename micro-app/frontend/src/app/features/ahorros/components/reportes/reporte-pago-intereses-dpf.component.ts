...
import { PagoInteresDpf, CuentaAhorroResumen } from '@core/models/ahorro.model';

@Component({
  selector: 'app-reporte-pago-intereses-dpf',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
      ...
      <div class="header">
        <div>
          <h1>
            ...
            Pago de Intereses - Depósito a Plazo Fijo
          </h1>
          <p class="subtitle">
            Reporte de pagos de intereses procesados en cuentas DPF
          </p>
        </div>
      </div>
      ...
      <mat-card class="filters-card">
        ...
        <mat-card-content>
          <form [formGroup]="filtrosForm" class="filters-form">
            <!-- Búsqueda de cuenta DPF -->
            <mat-form-field appearance="outline" class="full-width">
              <mat-label>Buscar Cuenta DPF (No. cuenta, nombre o DUI)</mat-label>
              <input matInput
                [formControl]="searchCuentaControl"
                [matAutocomplete]="autoCuenta"
                placeholder="Escriba para buscar..." />
              <mat-icon matSuffix>search</mat-icon>
              <mat-autocomplete #autoCuenta="matAutocomplete"
                (optionSelected)="onCuentaSelected($event.option.value)"
                [displayWith]="displayCuenta">
                @for (cuenta of cuentasDpf(); track cuenta.id) {
                  <mat-option [value]="cuenta">
                    {{ cuenta.noCuenta }} - {{ cuenta.nombreCliente }} ({{ cuenta.numeroDui }})
                  </mat-option>
                }
              </mat-autocomplete>
              @if (cuentaSeleccionada()) {
                <button matSuffix mat-icon-button (click)="limpiarCuenta($event)" matTooltip="Quitar filtro de cuenta">
            ...
            @if (cuentaSeleccionada()) {
              <div class="cuenta-info">
                <mat-icon>account_balance</mat-icon>
                <div>
                  <strong>{{ cuentaSeleccionada()!.noCuenta }}</strong> - {{ cuentaSeleccionada()!.nombreCliente }}
                  <span class="cuenta-detalle">{{ cuentaSeleccionada()!.tipoAhorro }} | Saldo: {{ cuentaSeleccionada()!.saldo | currency:'USD':'symbol':'1.2-2' }}</span>
                </div>
              </div>
            }
          ...
          <mat-card-content>
            ...
            <div class="summary">
              ...
              <div class="summary-item">
                ...
                <div>
                  <span class="summary-label">Total Monto DPF</span>
              ...
              <div class="summary-item primary">
                ...
                <div>
                  <span class="summary-label">Total Intereses Pagados</span>
                  <span class="summary-value">
                    {{ calcularTotalIntereses() | currency:'USD':'symbol':'1.2-2' }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div class="table-responsive">
              <table mat-table [dataSource]="datosPaginados()">
                ...
                <ng-container matColumnDef="numeroDui">
                  ...
                  <td mat-cell *matCellDef="let item">{{ item.numeroDui }}</td>
                </ng-container>
                ...
                <ng-container matColumnDef="monto">
                  <th mat-header-cell *matHeaderCellDef>Monto DPF</th>
                  <td mat-cell *matCellDef="let item">
                ...
                <ng-container matColumnDef="plazo">
                  ...
                  <td mat-cell *matCellDef="let item">{{ item.plazo }} días</td>
                </ng-container>
                ...
                <!-- Interés Pagado -->
                <ng-container matColumnDef="montoInteres">
                  <th mat-header-cell *matHeaderCellDef>Interés Pagado</th>
                  <td mat-cell *matCellDef="let item">
                    <strong>{{ item.montoInteres | currency:'USD':'symbol':'1.2-2' }}</strong>
                  </td>
                </ng-container>
        ...
        <mat-card class="empty-card">
          <mat-card-content>
            <div class="empty">
              ...
              <p>No se encontraron pagos de intereses DPF en el periodo seleccionado</p>
  ...
  styles: [`
    ...
    .cuenta-info mat-icon { color: #1976d2; }
    .cuenta-info div { display: flex; flex-direction: column; }
    .cuenta-detalle { font-size: 13px; color: #555; margin-top: 2px; }
...
export class ReportePagoInteresesDpfComponent implements OnInit {
  ...
  datosReporte = signal<PagoInteresDpf[]>([]);
  datosPaginados = signal<PagoInteresDpf[]>([]);
  exportandoExcel = signal(false);
  cuentasDpf = signal<CuentaAhorroResumen[]>([]);
  cuentaSeleccionada = signal<CuentaAhorroResumen | null>(null);

  // Búsqueda de cuenta
  searchCuentaControl = new FormControl('');
  ...
  ngOnInit(): void {
    ...
    this.searchCuentaControl.valueChanges
      .pipe(
        debounceTime(300),
        switchMap((val) => {
          if (typeof val === 'string' && val.length >= 2) {
            return this.cuentaAhorroService.getAll({
              buscar: val,
              lineaCodigo: 'DPF',
              limit: '20',
            });
      ...
      .subscribe((res) => this.cuentasDpf.set(res.data));
  }

  onCuentaSelected(cuenta: CuentaAhorroResumen): void {
    this.cuentaSeleccionada.set(cuenta);
  }

  displayCuenta(cuenta: CuentaAhorroResumen): string {
    return cuenta ? `${cuenta.noCuenta} - ${cuenta.nombreCliente}` : '';
  }

  limpiarCuenta(event: Event): void {
    event.stopPropagation();
    this.cuentaSeleccionada.set(null);
    this.searchCuentaControl.reset('');
    this.cuentasDpf.set([]);
  }
  ...
  generarReporte(): void {
    ...
    this.cuentaAhorroService.getPagoInteresesDpf(fechaDesde, fechaHasta, cuentaId).subscribe({
      next: (resp) => {
  ...
  calcularTotalIntereses(): number {
    return this.datosReporte().reduce(
      (sum, item) => sum + (item.montoInteres || 0),
      0,
    );
  }
  ...
  exportarExcel(): void {
    ...
    try {
      const datosExcel = this.datosReporte().map((item) => ({
        ...
        'Monto DPF': item.monto,
        'Tasa (%)': item.tasaInteres,
        'Plazo (días)': item.plazo,
        'Apertura': this.formatearFechaStr(item.fechaApertura),
        'Vencimiento': this.formatearFechaStr(item.fechaVencimiento),
        'Fecha Pago': this.formatearFechaStr(item.fechaPago),
        'Interés Pagado': item.montoInteres,
      }));
      ...
      datosExcel.push({
        ...
        'DUI': '',
        'Tipo Ahorro': 'TOTALES',
        'Monto DPF': this.calcularTotalMonto(),
        'Tasa (%)': '' as any,
        'Plazo (días)': '' as any,
        'Apertura': '',
        'Vencimiento': '',
        'Fecha Pago': '',
        'Interés Pagado': this.calcularTotalIntereses(),
      });

      const worksheet = XLSX.utils.json_to_sheet(datosExcel);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Pago Intereses DPF');

      const valores = this.filtrosForm.value;
      const fechaDesde = this.formatearFecha(valores.fechaDesde);
      const fechaHasta = this.formatearFecha(valores.fechaHasta);
      const nombreArchivo = `reporte-pago-intereses-dpf-${fechaDesde}-${fechaHasta}.xlsx`;
